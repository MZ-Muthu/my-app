...
exports.verifyOpticalsProductData = catchAsync(async (req, res, next) => {
...
// save json
exports.saveFilesAsJbgForOpticalProduct = catchAsync(async (req, res, next) => {
    if (req.body.statusType === 'create') {
...
exports.saveOpticalsProductToAWS = catchAsync(async (req, res, next) => {
    const s3 = new AWS.S3({
    ...
    if (req.body.statusType === 'create') {
        req.body.productDetails = await Promise.all(
            req.body.productDetails.map(async (el, index) => {
                ...
                const params = {
                    ...
                    Key: `${imageName}-hearingaid-product-banner.png`,
                ...
                const paramsImage = {
                    Bucket: process.env.AWS_BUCKET,
                    Key: `${imageName}-opticals-frames.png`,
                    ContentType: 'image/png',
                    Body: Buffer.from(el.frameImage)
                };
                ...
                try {
                    frameImage = await s3.upload(paramsImage).promise();
                    console.log(frameImage);
                    frameImage = frameImage.Location;
                } catch (error) {
                    console.log(error);
                    return next(
                        new AppError(
                ...
                const imageGallery = await Promise.all(
                    el.imageGallery.map(async (els2, index) => {
                        const params2 = {
                            Bucket: process.env.AWS_BUCKET,
                            Key: `${imageName}-hearingaid-product-gallery-${index}.png`,
                            ContentType: 'image/png',
                            Body: els2
                        };
                ...
                if (index === 0) {
                    req.body.frameImage = frameImage;
                }
                return {
                    ...el,
                    subDetails,
        ...
        const opticalsProduct = await opticalModel.findOne({
            userId: req.user._id,
            partnerId: req.docs._id,
            hiwnopmID: req.params.opticalId
        });
        ...
        req.body.productDetails = await Promise.all(
            req.body.productDetails.map(async (el, index) => {
                let eID = el.id;
                if (el.id === undefined) {
                    if (!el.bannerImage || !el.imageGallery || !el.frameImage) {
                        return next(
                            new AppError(
                ...
                let image, imageGallery, frameImage, filteredData;
                if (!el.bannerImage || !el.imageGallery || !el.frameImage) {
                    [filteredData] = await Promise.all(
                        opticalsProduct.productDetails.filter((els) => {
                ...
                if (el.bannerImage) {
                    const params = {
                        Bucket: process.env.AWS_BUCKET,
                        Key: `${imageName}-opticals-product-banner.png`,
                        ContentType: 'image/png',
                        Body: el.bannerImage
                    };
                ...
                if (el.frameImage) {
                    ...
                    try {
                        frameImage = await s3.upload(paramsImgae).promise();
                        frameImage = frameImage.Location;
                    } catch (error) {
                        return next(
                            new AppError(
                    ...
                    frameImage = filteredData.frameImage;
                }
                if (el.imageGallery) {
                    imageGallery = await Promise.all(
                        el.imageGallery.map(async (els, index) => {
                            const params2 = {
                                Bucket: process.env.AWS_BUCKET,
                                Key: `${imageName}-opticals-product-gallery-${index}.png`,
                                ContentType: 'image/png',
                                Body: els
                            };
                ...
                const subDetails = await Promise.all(
                    el.subDetails.map(async (els, index2) => {
                        let eID2 = els.id;
                        if (els.id === undefined) {
                            els.hiwoppspdsID = await encryptID(
                                process.env.OPTICAL_SECRET
                            );
                            eID2 = els.hiwoppspdsID;
                        }
...
exports.orderOpticals = catchAsync(async (req, res, next) => {
        ...
        product.productDetails.map(async (el) => {
            if (el.color === req.body.color) {
                const [size] = await Promise.all([
                    el.subDetails.find((els) => els.size === req.body.size)
                ]);
                obj = {
                    color: el.color,
                    size,
    ...
    req.body.price = obj.size.price;
    req.body.discountPrice = obj.size.discountPrice;
    req.body.hiwonsmID = await encryptID(process.env.OPTICAL_SECRET);
    ...
    req.body.glassRimType = product.glassRimType;
    req.body.frameType = product.frameType;
    req.body.bannerImage = obj.bannerImage;
    req.body.imageGallery = obj.imageGallery;
    req.body.product = product;
    if (!obj.color || !obj.size.size) {
        return next(new AppError('Product not found.', 404));
    }
...
// check if all data was visible
exports.uploadSinglAndMultipleWithFrameImage = upload.fields([
    {
        name: 'image',
        maxCount: 1
    },
...
// get image values
exports.getUploadImageFrameData = catchAsync(async (req, res, next) => {
    if (req.body.imageStatus === 'create') {
        req.files.imageGallery = await Promise.all(
            req.files.imageGallery.map((el) => el.buffer)
        );
        if (req.files.frame[0].mimetype !== 'image/png') {
            return next(new AppError('Frame Images should be png', 400));
        }
        return res.status(200).json({
            status: 'Success',
            image: req.files.image[0].buffer,
            imageGallery: req.files.imageGallery,
            frame: req.files.frame[0].buffer
        });
    } else if (req.body.imageStatus === 'update') {
        let image, imageGallery, frame;
        ...
        if (req.files.frame?.length && req.files.frame[0]?.buffer) {
            frame = req.files.frame[0].buffer;
        } else {
            frame = undefined;
        }
...
// get all product
exports.getAllOpticalProducts = factoryController.getAllFilter(opticalModel);

// get a product
exports.sendAllFilter = factoryController.sendAllFilter();

// assign data for get a product
exports.assignDataForGetAProduct = (req, res, next) => {
    req.searchQuery = {
        hiwnopmID: req.params.productId
    };
    req.queryPopulate = [
        {
            path: 'partner',
...
exports.getAOpticalProduct = factoryController.findOneWithPopulate(
    opticalModel,
    'Product not found.'
);

// send json for find one
exports.sendJsonForFindOneWithPopulate =
    factoryController.sendJsonForFindOneWithPopulate();
