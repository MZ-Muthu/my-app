...
exports.sendQuotesToNearbyServices = catchAsync(async (req, res, next) => {
    ...
    if (!services.length) {
        return next(
            new AppError(
                "Sorry we couldn't find any ambulance providers in 20km radius.",
                404
            )
        );
    }
...
exports.assignDataForServiceProvers = catchAsync(async (req, res, next) => {
    const quote = await ambulanceQuotesModel.findOne({
    ...
    if (req.params.status === 'responded') {
        ...
        req.body.respondedBy = req.user.role === 'admin' ? 'admin' : 'partner';
        ...
        req.body.status = req.params.status;
        req.body.completedBy = req.user.role === 'admin' ? 'admin' : 'partner';
...
exports.updateServiceProviders =
    factoryController.updateById(ambulanceQuotesModel);

// assing data for update remaing status
exports.assingDataforUpdateRemainingStatus = (req, res, next) => {
    if (req.params.status !== 'accepted') {
        return res.status(200).json({ status: 'Success' });
    }
    req.updateAllSearchQuery = {
        batch: req.uDocs.batch,
        userId: req.user._id,
        status: { $ne: 'accepted' }
    };
    req.updateAllData = { status: 'missed' };
    return next();
};

// update ambulance remaing ambulance status
exports.updateRemainingAmbualceStatus =
...
exports.responseProvidersQuote = catchAsync(async (req, res, next) => {
    const quote = await ambulanceQuotesModel.findOne({
        userId: req.user._id,
        hiwnrqID: req.params.quoteId
    });
    ...
    if (req.params.status === 'accepted') {
        ...
        req.body.quoteAmount = req.body.quote;
        req.body.userRespondedBy = req.user.role === 'admin' ? 'admin' : 'user';
        req.body.userRespondTime = Date.now();
    } else if (req.params.status === 'rejected') {
        if (quote.status !== 'responded') {
            return next(
                new AppError(
                    `This request was already ${quote.status}.So we can't accept this request.`,
                    400
                )
            );
        }
...
exports.verifyAmbulanceDriver = catchAsync(async (req, res, next) => {
    ...
    const checkDriverAndAmbualnce = await ambulanceQuotesModel.findOne({
        status: 'assigned',
        $or: [
            {
                'assignedAmbulanceDriver.id': drivers[0]._id
            },
            {
                'assignedAmbulance.id': ambulance[0]._id
            }
        ]
    });

    if (checkDriverAndAmbualnce) {
        return next(
            new AppError(
                'Your ambulance or driver is already assigned other ambulance reqeust.',
                400
            )
        );
    }
    ...
    const AssignDriver = await ambulanceQuotesModel.findOneAndUpdate(
        ...
        },
        {
            ...
            assignedAmbulanceDriver: {
                ...
                driverProfile: drivers[0].driverProfile
            },
            assignedAmbulance: {
                ambulanceNumber: ambulance[0].ambulanceNumber,
                location: ambulance[0].location,
                services: ambulance[0].services,
                id: ambulance[0]._id
            }
        },
...
// assign partner search data
exports.assignPartnerSearchUpdateData = (req, res, next) => {
    req.updateOneSearch = {
        userId: req.user._id,
        status: 'accepted',
        for: 'Ambulance Service'
...
exports.checkValidUserAmbulanceServices = async (req, res, next) => {
    let services = req.cookies.services?.split(',') ?? false;
    if (!services || !services[0].length) {
        return res.redirect('/ambulance/ambulance-service');
    }
    req.body.services = await Promise.all(services.filter((el) => !!el));
    return next();
};
                ...
                waiting: [
                    {
                        $match: {
                            $and: [
                                { 'list.status': { $ne: 'accepted' } },
                                { 'list.status': { $ne: 'responded' } },
                                { 'list.status': { $ne: 'assigned' } },
                                { 'list.status': { $ne: 'rejected' } },
                                { 'list.status': { $ne: 'canceled' } },
                                { 'list.status': { $ne: 'completed' } },
                                { 'list.status': { $ne: 'missed' } }
                            ]
                        }
                    }
                ],
...
// send json fro ambulance quote
exports.sendJsonForAmbulanceQuotes = (req, res) =>
    res.status(200).json({ status: 'Success', docs: req.body.quotes });
