...
exports.cronJob = catchAsync(async (req, res, next) => {
    ...
    } else if (req.body.scheduleDate.scheduleType === 'everyday') {
        req.body.scheduleDate.startDate = undefined;
        req.body.scheduleDate.endDate = undefined;
    }
    await Promise.all(
        req.body.reminderTime.map(async (el) => {
...
// delete pill remineers
exports.deletePillReminders = catchAsync(async (req, res, next) => {
    const [reminder, schedule] = await Promise.all([
        pillRemainderModel.findOneAndDelete({
            userId: req.user._id,
            hiwprmEID: req.params.reminderId
        }),
        schedulterModel.updateMany(
            {
                reminderId: req.params.reminderId,
...
// assign Datafor send sms
exports.getPillReminderAndSendSMS = catchAsync(async (req, res, next) => {
    const [remind] = await pillRemainderModel.aggregate([
        {
            $match: {
                userId: req.user._id,
                hiwprmEID: req.params.reminderId,
                'reminderDetails.status': 'waiting'
            }
        },
        {
            $lookup: {
    ...
    if (!remind) {
        return next(
            new AppError(
                'Reminder not found! or you have already completed the pill. wait for next reminder.',
                404
            )
        );
    }
    if (!remind.member.length) {
        return next(new AppError('Member not found!', 404));
    }
    await Promise.all([
        pillRemainderModel.findByIdAndUpdate(remind._id, {
            $push: {
                reminderHistory: {
                    targetTime: 'remind.reminderDetails.time',
                    takenTime: Date.now(),
                    count: remind.reminderHistory.length
                }
            },
            reminderDetails: { status: 'inActive' }
        }),
        sendEmail({
            email: req.user.email,
            subject: 'Pill Reminder Neutral',
            message: `Your registerd persons taked pill.`
        })
        // sendEmail({
        //     email: remind.intimationPerson.personPhone,
        //     subject: 'Pill Reminder intimation person',
        //     message: `Your father pill takend pills.`
        // })
    ]);
    return res.status(200).json({ status: 'Success' });
});

// reminder send json for delte
exports.sendJsonForDeleteJson = (req, res, next) => {
    return res.status(200).json({ status: 'Success' });
};
