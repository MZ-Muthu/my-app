...
// get all servie
exports.getAllHomecareSerive = factoryController.getFindAllWithPopulateFilter(
...
// find data by id
exports.findOneHomeCareservice = factoryController.findOneWithPopulate(
    homecareservicesserviceModel
);
...
exports.getaHomecareService = catchAsync(async (req, res, next) => {
    if (req.body.findOnePopulateDocs.company[0].status !== 'accepted') {
        return next(new AppError('Service not found.', 404));
    }
    const [filterdData, address] = await Promise.all([
        homecareservicesserviceModel.aggregate([
            ...
            {
                $unwind: '$relatedProduct'
            },
            {
                $project: {
                    bannerImage: '$relatedProduct.bannerImage',
                    serviceName: '$relatedProduct.serviceName',
                    description: '$relatedProduct.description',
                    priceFrom: '$relatedProduct.priceFrom',
                    priceTo: '$relatedProduct.priceTo',
                    id: '$relatedProduct.hiwhcssms'
                }
            }
        ]),
    ...
    req.body.relatedData = filterdData;
    req.body.address = address;
    return next();
});

// send json for get a hoemcare service
exports.sendJsonForHomecareServices = (req, res) =>
    res.status(200).json({
        status: 'Success',
...
// assign data for get a service
exports.assignGetAServiceDataForGetProduct = (req, res, next) => {
    req.searchQuery = { hiwhcssms: req.params.serviceId };
    req.queryPopulate = [
        {
            path: 'company',
            select: 'status company openTime closeTime centerPhone centerLandLine homecare'
        }
    ];
    return next();
};
...
exports.verifyandAssignDataForHomecareServiceApplies = catchAsync(
    async (req, res, next) => {
        ...
        req.body.hoemcareEID = homecare.hiwhcssms;
        req.body.homecareID = homecare._id;
        req.body.company = homecare.partners.company;
        req.body.phone = homecare.partners.centerPhone;
        ...
        homecare.company = req.body.company;
        homecare.phone = req.body.phone;
        req.body.createdAt = Date.now();
...
// get homecare service data
exports.filterHomecaredata = catchAsync(async (req, res, next) => {
    const data = [];
    await Promise.all(
        req.body.findAllFilter.map((el) => {
            if (el.company[0]?.status === 'accepted')
                return data.push({
                    ...el._doc,
                    company: el.company[0].company
                });
        })
    );

    req.body.findAllFilter = data;
