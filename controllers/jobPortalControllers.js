...
exports.updateUserPesonalDetails = catchAsync(async (req, res, next) => {
    if (
        ...
        if (req.query.deleteuserdata === 'yes') {
            return res.status(202).json({
                status: 'Successed'
            });
        }
...
exports.checkifTheUserAlreadyExist = catchAsync(async (req, res, next) => {
    ...
    req.body.docs = userData;
    return next();
});

// send json for get user data
exports.sendJsonForUserProfileDocs = (req, res) =>
...
exports.getCategories = catchAsync(async (req, res, next) => {
    const [getaJob, categories] = await Promise.all([
        jobportalModel.findOne({
            userId: req.user._id,
            hiwjbmID: req.params.jobId
        }),
...
exports.filterVerifiedPartners = catchAsync(async (req, res, next) => {
    ...
    await Promise.all(
        req.body.findAllFilter.map((el) => {
            ...
            if (el.partner && el.partnerDetails.length) return filter.push(el);
        })
    );
    req.body.findAllFilter = filter;
    return next();
});
...
exports.getAJob = catchAsync(async (req, res, next) => {
    ...
    req.body.docs = filterdData.partner[0]?.job
        ? filterdData.partner[0].job
        : filterdData.user[0]?.job && filterdData.user[0].job;
    return next();
});

// send josn fof get a job
exports.sendJsonForAJob = (req, res) =>
    res.status(200).json({
        status: 'Success',
        job: req.body.docs
    });
...
// assing data for user
exports.assignDataForUser = (req, res, next) => {
...
// assign data for get a job categore
exports.assignDataforGetAJobCategorie = (req, res, next) => {
    req.searchQuery = { name: req.params.name };
    return next();
};

// get a categories
exports.getAJbobCategorie = factoryControllers.findOne(jobCategories);

// send json for get one
exports.sendJsonforGetOne = factoryControllers.sendJsonForFindOne();

// get all job categores
exports.getJobCategories = factoryControllers.getAll(jobCategories);
...
// assign data for delte job
exports.assingDataForDelteAJob = (req, res, next) => {
    req.searchQuery = {
        userId: req.user._id,
        from: req.params.from,
        hiwjbmID: req.params.jobId
...
// delete job
exports.deleteJob = factoryControllers.deleteOne(
    jobportalModel,
    'Job not found!'
);

// send json for delete one
exports.sendJsonFroDeteleOne = factoryControllers.sendJsonForDeleteOne();
