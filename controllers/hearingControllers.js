...
// send json for orders
exports.sendJsonForSendOrders = (req, res) =>
    res.status(200).json({ status: 'Success', docs: req.body.orders });
...
// assign user render  page
exports.assingUserRenderPage = (req, res, next) => {
    if (req.params.type === 'shop')
        req.body.render = 'user/hearingAid/hearingaidShopHome';
    else if (req.params.type === 'hospital')
        req.body.render = 'user/hearingAid/hearingaidHospitalHome';
    else if (req.params.type === 'repairStore')
        req.body.render = 'user/hearingAid/hearingaidRepairStoreHome';
...
exports.checkForShop = (req, res, next) => {
    if (req.params.type === 'shop')
        return res.status(200).json({
            status: 'Success',
            docs: req.body.product
        });
    return next();
};
...
exports.getMyhearringAidOrder = catchAsync(async (req, res, next) => {
    const [order] = await orderHearingaidModel.aggregate([
        {
            $match: {
                userId: req.user._id
            }
...
exports.getAllHearingaidHospitalBooking = catchAsync(async (req, res, next) => {
    const [bookings] = await bookHearingAidHospitalModel.aggregate([
        {
            $match: { userId: req.user._id }
        },
        {
    ...
    req.body.docs = bookings;
    return next();
});

// change order to booking
exports.changeDocstoBooking = (req, res, next) => {
    req.body.bookings = req.body.findAllFilter;
    req.body.findAllFilter = undefined;
    return next();
};
...
exports.getHearingaidProductsData = catchAsync(async (req, res, next) => {
    const id = req.cookies['pdohdId'];
    if (!id) {
        return next(
            new AppError('Something went wrong please try again.', 404)
        );
    }
    const [product] = await hearingAidProductModel.aggregate([
        {
            $match: {
                hiwhapnsID: id
            }
    ...
    await Promise.all(
        product.productDetails.map(async (el) => {
            if (el.hiwhapdsID === req.body.color) {
                color = el.color;
                imageGallery = el.imageGallery;
                ...
                if (!req.body.size) {
                    ...
                    console.log(el.hiwhaicscpID);
                    [size] = await Promise.all([
                        el.subDetails.find(
                            (el) => el.hiwhaicscpID === req.body.size
                        )
                    ]);
                    if (!size) {
                        return next(new AppError('Product not found', 404));
