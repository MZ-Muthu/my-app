...
exports.assignDeaddictionSaveQuery = catchAsync(async (req, res, next) => {
    ...
    console.log(req.body.addiction);
    ...
    req.body.deaddictionCenterDetails = {
        ...
        company: req.docs.company,
        bannerImage: req.docs.bannerImage
    };
...
exports.getAllDeaddictionCenter =
    factoryController.getFindAllWithPopulateFilter(deaddictionModel);
...
exports.assignDataForGetADeaddictionSErvice = catchAsync(
    async (req, res, next) => {
        const [center] = await deaddictionModel.aggregate([
            ...
            },
            {
                $lookup: {
                    ...
                    pipeline: [
                        {
                            $match: { hiwdacmID: { $ne: req.params.centerId } }
                        },
                        { $limit: 10 },
                        {
                            $project: {
        ...
        req.body.docs = center;
        return next();
    }
);
// send json fro center
exports.sendJsonForDeaddictionServices = (req, res) =>
    res.status(200).json({
        status: 'Success',
        docs: req.body.docs
    });
...
exports.getAllMyDeaddictionBookings = catchAsync(async (req, res, next) => {
    const docs = await applydeaddictionModel.aggregate([
        {
            $match: {
                userId: req.user._id
            }
