...
export let updateMedicalMarket = async (data) => {
    try {
        await axios({
            method: 'PATCH',
            url: `/api/v1/market/update-market`,
            data
        })
            .then((res) => {
                if (res.data.status === 'Success') {
                    swal(
                        'Success',
                        'Your Market profile updated successfully.',
                        'success'
                    );
...
// update medical market
export let createNewMarketProduct = async (data) => {
    try {
        await axios({
            method: 'POST',
            url: `/api/v1/market/new-medical-market-product`,
            data
        })
            .then((res) => {
                if (res.data.status === 'Success') {
...
export let updateMarketProduct = async (data, id) => {
    try {
        await axios({
            method: 'PATCH',
            url: `/api/v1/market/update-medical-market-product/${id}`,
            data
        })
            .then((res) => {
                if (res.data.status === 'Success') {
...
export let updateMarketProductOrderStatus = async (status, id) => {
    try {
        swal({
            title: 'Are you sure?',
            ...
            .then(async (willDelete) => {
                if (willDelete) {
                    await axios({
                        method: 'PATCH',
                        url: `/api/v1/market/update-order-status/${status}/${id}`
                    })
                        .then((res) => {
...
export let updateMarketProductQuoteStatus = async (status, id) => {
    try {
        swal({
            title: 'Are you sure?',
            ...
            .then(async (willDelete) => {
                if (willDelete) {
                    ...
                    await Swal.fire({
                        ...
                        await axios({
                            method: 'PATCH',
                            url: `/api/v1/market/update-quote-status/${id}/`,
                            data: value.value
                        })
                            .then((res) => {
                                if (res.data.status === 'Success') {
